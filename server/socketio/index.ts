      ...
      socket.on('admin_send', async data => {
        ...
        if(data.image){
          ...
          s3.upload(params, async (err: any, uploadedData: any) => {
            ...
            console.log(`File uploaded to AWS S3 successfully. ${uploadedData.Location}`);
            ...
            socket.to(`customer_service_room_${data.receiver}`).emit('to_customer', newMessage);
          });
        }else{
      ...
      socket.on('customer_send', async data => {
        ...
        if(data.username === ""){
          ...
          if(messageItem){
            ...
            messageItem = c.find({'userNo': { $exists: true}}).sort({'userNo': -1}).limit(1);
