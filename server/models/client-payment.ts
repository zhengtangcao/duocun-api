...
export class ClientPayment extends Model {
  ...
  hello(req: Request, res: Response) {
    log.debug("calling hello debug");
    log.info("calling hello info");
    log.info("calling hello error");
    res.send({ msg: "Hello" });
  }
  ...
  snappayPayOrder(req: Request, res: Response) {
    ...
    if (order.type === OrderType.MOBILE_PLAN_SETUP) {
      returnUrl =
        "https://duocun.com.cn/cell?clientId=" +
        clientId +
        "&paymentMethod=" +
        paymentMethod +
  ...
  stripeRefund(req: Request, res: Response) {
    ...
    stripe.refunds.create({ charge: chargeId }, function (err: any, re: any) {
      ...
      if (err) {
        ...
        res.send(
          JSON.stringify({ status: re.status, refundId: re.id }, null, 3)
        );
      }
    });
  }
